...
UG_GUI gui;
/** A screen sized buffer */
uint32_t lcdbuf_gui[LCD_HDOT * LCD_VDOT] __attribute__((section(".SDRAM1")));
...
void lcd_init(void)
{
    ...
    LCD_initStruct.DataSource = (uint32_t)lcdbuf_gui;
 ...
 * @brief   软件描点
 */
static void put_px(int16_t x, int16_t y, uint32_t color)
{
    lcdbuf_gui[y * 480 + x] = color;
}
void fill_color(uint16_t x1,uint16_t y1,uint16_t x2,uint16_t y2,uint32_t color)
{
	int32_t x;
